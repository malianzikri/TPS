...
public class TPSUpload {
    ...
    public String excuteParsingDok(String jnsDok, String fstream, String sementara) throws Exception {
        ...
        if (fstream.contains("</ETA>")) {
            ...
            String hasil = contSHL.parseDocument(fstream);
            if (hasil.equalsIgnoreCase("SUKSES")) {
    ...
    public String upload_TPS(String document, String fStream, String usernamews, String passwordws) throws ClassNotFoundException {
        ...
        String dirHistoy = PROPERTIES.getProperty("tps.history.outbox") + File.separator + usernamews;
        String DirError = PROPERTIES.getProperty("tps.error.outbox") + File.separator + usernamews;
        ...
        try {
            ...
            if (document.equalsIgnoreCase("UploadMohonPLP")) {
                ...
                if (fStream.contains("<KD_TPS>PLDC</KD_TPS>") || usernamews.equalsIgnoreCase("KOJA") || usernamews.equalsIgnoreCase("CART")) {
                    ...
                    result = excuteParsingDok(document, fStream, "");
                    message += log.LogSuccess(result.toString()) + "\r\n";
                    //Checking Result
            ...
            try {
                if (result.contains("Proses Berhasil") || result.contains("PROSES SUCCESS")) {
                    ...
                    moveFile = mv.isExecute(dirOutbox + File.separator + namafile, dirDaily + File.separator + namafile);
                ...
                logdb.excuteLogSucces(usernamews, passwordws, document, result, dirDaily + File.separator + namafile, fStream);
            } catch (Exception e) {
                logdb.excuteLogError(e.getMessage(), "TPSUpload", usernamews);
                messageErr += log.LogError(e.getMessage()) + "\r\n";
            }

        } catch (Exception ex) {
            logdb.excuteLogError(ex.getMessage(), "TPSUpload", usernamews);
